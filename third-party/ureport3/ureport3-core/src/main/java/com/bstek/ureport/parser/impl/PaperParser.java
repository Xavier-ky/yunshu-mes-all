...
public class PaperParser implements Parser<Paper> {
	...
	public Paper parse(Element element) {
		...
		if(StringUtils.hasText(columnEnabled)){
		...
		if(StringUtils.hasText(htmlReportAlign)){
		...
		if(StringUtils.hasText(htmlIntervalRefreshValue)){

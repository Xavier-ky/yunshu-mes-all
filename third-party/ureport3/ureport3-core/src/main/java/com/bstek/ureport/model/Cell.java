...
public class Cell implements ReportCell {
	...
	public void doFormat(){
		...
		if(StringUtils.hasText(customFormat)){
			format=customFormat;
		}
		if(!StringUtils.hasText(format) || data==null || !StringUtils.hasText(data.toString())){
			return;
		}
	...
	private void doComputeConditionProperty(Context context){
		...
		for(ConditionPropertyItem item:conditionPropertyItems){
			...
			if(StringUtils.hasText(item.getNewValue())){
				this.data=item.getNewValue();
				this.formatData=item.getNewValue();
			}
			if(StringUtils.hasText(item.getLinkUrl())){
				linkUrl=item.getLinkUrl();
				if(StringUtils.hasText(item.getLinkTargetWindow())){
	...
	public void doDataWrapCompute(Context context){
		...
		if(!StringUtils.hasText(dataText) || dataText.length()<2){
			return;
		}

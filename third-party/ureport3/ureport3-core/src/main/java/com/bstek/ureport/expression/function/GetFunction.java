...
public class GetFunction implements Function {
	...
	public Object execute(List<ExpressionData<?>> dataList, Context context,Cell currentCell) {
		...
		if(dataList.size()==1){
			...
			if(d instanceof ObjectExpressionData exprData){
	...
	private void buildList(ExpressionData<?> d, List<Object> list) {
		...
		}else if(d instanceof ObjectListExpressionData listData){
            list.addAll(listData.getData());
		}else if(d instanceof BindDataListExpressionData listData){

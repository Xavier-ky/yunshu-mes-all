  ...
  <div class="app-container">
    <el-row>
      <el-col :span="24" style="text-align: right;">
        <el-button icon="el-icon-refresh" size="default" @click="resetQuery">刷新</el-button>
      </el-col>
    </el-row>
    <el-table v-loading="loading" :data="itemreceptlineList" style="overflow-x: auto; width: 100%;" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" width="100px" v-if="optType != 'view'" class-name="small-padding fixed-width">
        <template #default="scope">
          ...
          <el-button
            ...
            v-hasPermi="['mes:wm:itemrecpt:print']"
          >标签打印</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <!-- 入库单行查看界面 -->
    <el-dialog :title="linetitle" v-model="lineopen" width="960px" append-to-body>
...
<script>
import { listItemreceptdetail, getItemreceptdetail, delItemreceptdetail, addItemreceptdetail, updateItemreceptdetail } from "@/yunshu-ui/api/mes/wm/itemreceptdetail";
import { listItemrecptline, listWithDetail, getItemrecptline } from "@/yunshu-ui/api/mes/wm/itemrecptline";
import {getTreeList} from "@/yunshu-ui/api/mes/wm/warehouse"
import {getByTemplateType, getTemplate} from "@/yunshu-ui/api/print/template";
import { getBarcodeUrl } from '@/yunshu-ui/api/mes/wm/barcode';
export default {
  name: "Itemreceptdetail",
  props: {
    recptId: null,
    optType: null,
  },
  data() {
    return {
      ...
      queryParams: {
        pageNum: 1,
        pageSize: 10,
        lineId: null,        recptId: this.recptId,        itemId: null,        itemCode: null,        itemName: null,        specification: null,        unitOfMeasure: null,        quantity: null,        batchCode: null,        warehouseId: null,        warehouseCode: null,        warehouseName: null,        locationId: null,        locationCode: null,        locationName: null,        areaId: null,        areaCode: null,        areaName: null,      },
      // 表单参数
      form: {},
      // 表单校验
      rules: {
  ...
  methods: {
    ...
    getListWithDetail() {
      ...
      listWithDetail(this.queryParams).then(response => {
        this.itemreceptlineList = response.rows;
    ...
    reset() {
      this.form = {
      detailId: null,        lineId: null,        recptId: this.recptId,   materialStockId: null,     itemId: null,        itemCode: null,        itemName: null,        specification: null,        unitOfMeasure: null,  unitName: null,      quantity: null,        batchCode: null,        warehouseId: null,        warehouseCode: null,        warehouseName: null,        locationId: null,        locationCode: null,        locationName: null,        areaId: null,        areaCode: null,        areaName: null,        remark: null,        attr1: null,        attr2: null,        attr3: null,        attr4: null,        createBy: null,        createTime: null,        updateBy: null,        updateTime: null      };
      this.warehouseInfo = [];
      this.resetForm("form");
    },
    ...
    handleAdd(row) {
      ...
      this.open = true;
      this.title = "添加物料入库单明细";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const detailId = row.detailId || this.ids
      getItemreceptdetail(detailId).then(response => {
        this.form = response.data;
        this.warehouseInfo[0] = response.data.warehouseId;
        this.warehouseInfo[1] = response.data.locationId;
        this.warehouseInfo[2] = response.data.areaId;
        this.open = true;
        this.title = "修改物料入库单明细";
      });
    },
    /** 提交按钮 */
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.detailId != null) {
            updateItemreceptdetail(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getListWithDetail();
            });
          } else {
            addItemreceptdetail(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除物料入库单明细编号为"' + detailIds + '"的数据项？').then(function() {
        return delItemreceptdetail(detailIds);
      }).then(() => {
        this.getListWithDetail();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('wm/itemreceptdetail/export', {
        ...this.queryParams
      }, `itemreceptdetail_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>

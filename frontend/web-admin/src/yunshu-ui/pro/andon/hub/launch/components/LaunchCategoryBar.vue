<template>
  <div class="andon-launch-category-row">
    <button
      v-for="cat in cards"
      :key="cat.key"
      type="button"
    ...
    >
      <div class="andon-category-insight-card__top">
        ...
        <span class="andon-category-insight-card__pct">今日</span>
      </div>
...
<script>
import { CATEGORIES, categorizeReason } from "../../../utils/category";

export default {
  name: "LaunchCategoryBar",
  props: {
  ...
  computed: {
    cards() {
      ...
      (this.todayRecords || []).forEach((r) => {
        counts[categorizeReason(r.andonReason)] += 1;
      });
      const total = (this.todayRecords || []).length || 1;

        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="工作站" prop="workstationName">
              <el-input v-model="form.workstationName" placeholder="请选择工作站" ><template #append><el-button icon="el-icon-search" @click="handleWorkstationSelect"></el-button></template>
              </el-input>
            </el-form-item>
...
<script>
import { listProtask, getProtask, delProtask, addProtask, updateProtask } from "@/yunshu-ui/api/mes/pro/protask";
...
export default {
  ...
  watch: {
    workorderId(val) {
      this.queryParams.workorderId = val;
    },
    processId(val) {
  ...
  methods: {
    ...
    handleAdd() {
      this.reset();
      const qty = Number(this.workorderQty) || 100;
      const start = new Date();
      start.setHours(8, 0, 0, 0);
      this.form.quantity = qty;
      this.form.duration = 1;
      this.form.startTime = `${start.getFullYear()}-${String(start.getMonth() + 1).padStart(2, "0")}-${String(start.getDate()).padStart(2, "0")} 08:00:00`;
      this.calculateEndTime();
      this.form.taskName = this.processName ? `${this.processName}任务` : null;
      this.open = true;
      this.title = "添加生产任务";
    },
    /** 修改按钮操作 */

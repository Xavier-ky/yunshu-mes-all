...
<script>
import { gantt } from "@/yunshu-ui/lib/dhtmlxgantt.js";
export default {
  name: 'GanttChar',
  data() {
    return {
  ...
  mounted: function () {
    ...
    this.addTodayLine();
    this.lastTasksKey = JSON.stringify(this.tasks);
  ...
  watch: {
    tasks: {
      deep: true,
      handler(newVal) {
        if (!this.ganttReady) return;
        const key = JSON.stringify(newVal);
        if (key === this.lastTasksKey) return;
        this.lastTasksKey = key;
    ...
    initConfig() {
     ...
     // gantt.config.subscales = [{unit: "month", step: 1, date: "%Y,%F"}];
      ...
      gantt.attachEvent("onAfterTaskUpdate", (id) => {
        if (this.optType !== 'edit') return;
        this.$emit('task-changed', id);
      });
...
<style>
  ...
  /* 排产派工主页：仅提升甘特展示层次，不改变甘特行为或时间配置 */
  .scheduling-command-center .gantt_grid_scale,
  .scheduling-command-center .gantt_task_scale {
    border-color: #d8e1ec !important;
    background: #f6f8fb;
  }

  .scheduling-command-center .gantt_scale_cell {
    color: #526176;
    font-weight: 600;
    border-color: #e0e7f0 !important;
  }

  .scheduling-command-center .gantt_grid_head_cell {
    color: #3a4860;
    font-weight: 600;
    border-color: #d8e1ec !important;
    background: #f2f6fb;
  ...
  .scheduling-command-center .gantt_grid_data .gantt_row,
  .scheduling-command-center .gantt_task_row {
    border-color: #edf1f6 !important;
  }

  .scheduling-command-center .gantt_grid_data .gantt_row.odd,
  .scheduling-command-center .gantt_task_row.odd {
    background: #fbfcfe;
  }

  .scheduling-command-center .gantt_grid_data .gantt_row.odd:hover,
  .scheduling-command-center .gantt_grid_data .gantt_row:hover,
  .scheduling-command-center .gantt_task_row:hover {
    background: #eef5ff !important;
  }

  .scheduling-command-center .gantt_task_line {
    height: 18px !important;
    line-height: 18px !important;
    margin-top: 7px;
    border: 1px solid rgba(23, 105, 224, 0.16) !important;
    box-shadow: 0 2px 5px rgba(23, 105, 224, 0.16);
  }

  .scheduling-command-center .gantt_task_content {
    font-weight: 600;
  }

  .scheduling-command-center .gantt_marker.today {
    width: 2px !important;
    background: #e28a21 !important;
    opacity: 0.9;
  }

  .scheduling-command-center .weekend {
    background: rgba(238, 243, 249, 0.8) !important;
  }
</style>
<template>
  <div class="cal-workbench-toolbar">
    ...
    <div class="cal-workbench-toolbar__filters">
      ...
      <el-input
        :model-value="queryParams.planCode"
        clearable
        placeholder="计划编号"
        class="cal-workbench-toolbar__select"
        @keyup.enter="$emit('search')"
        @update:model-value="$emit('update:planCode', $event)"
      />
      <el-input
        :model-value="queryParams.planName"
        clearable
        placeholder="计划名称"
        class="cal-workbench-toolbar__select"
        @keyup.enter="$emit('search')"
        @update:model-value="$emit('update:planName', $event)"
      />
      <el-date-picker
      ...
      <el-button link type="primary" @click="$emit('navigate-tab', 'team')">班组</el-button>
    </div>
  </div>
</template>

  ...
  <div class="operator-hero-bar">
    ...
    <div class="operator-hero-actions">
      ...
      <el-button
        ...
        @click="$emit('go-analytics')"
      >
        <span class="operator-ui-btn__label">分析中心</span>
      </el-button>
    </div>
  </div>
</template>
...
export default {
  ...
  props: {
    ...
    showAnalyticsLink: { type: Boolean, default: false },
  },
  ...
  computed: {
    cards() {
      ...
      return [
        { key: "pending", label: "待检", display: s.pendingCount ?? 0, tone: "tone-blue" },
        { key: "today", label: "今日完成", display: s.todayFinished ?? 0, tone: "tone-cyan" },
        { key: "pass", label: "合格率", display: `${s.passRate ?? 0}%`, tone: "tone-green" },
        { key: "defect", label: "不良批次", display: s.defectBatchCount ?? 0, tone: "tone-red" },
        { key: "disposition", label: "待处置", display: s.pendingDisposition ?? 0, tone: "tone-indigo" },
      ];
    },
  },
  mounted() {

<template>
    <div class="app-container" :class="{ 'hub-embed-panel': embedMode }">
      ...
      <el-row :gutter="16" class="card-grid">
        <el-col :xs="24" :sm="12" :md="8" :lg="6">
          <el-card class="chartCard card-add-wrap" shadow="hover">
            <div class="card-add" @click="handleAdd">
              <el-icon class="card-add-icon"><Plus /></el-icon>
              <span class="card-add-text">新建图表</span>
            </div>
          </el-card>
        </el-col>
        <el-col
        ...
        >
          <el-card class="chartCard">
            <template #header>
              <div class="clearfix chart-card-header">
                <span class="chart-card-title">{{ item.chartName }}</span>
                <div class="chart-card-actions">
                  <el-tooltip effect="dark" content="编辑图表" placement="top">
                    <el-icon class="chart-action-icon" @click="handleEdit(item)"><Edit /></el-icon>
                  </el-tooltip>
                  <el-tooltip effect="dark" content="删除图表" placement="top">
                    <el-icon class="chart-action-icon" @click="handleDelete(item)"><Delete /></el-icon>
                  </el-tooltip>
                </div>
              </div>
            </template>
            <div class="chart-card-body">
              <chart :chart-options="item.options" :api="item.api || ''" />
            </div>
          </el-card>
        </el-col>
      </el-row>
  ...
  <script>
  import { listChart,getChart,addChart,updateChart,delChart } from "@/yunshu-ui/api/mes/report/chart";

...
@Repository
public class DvMaintenRecordLineRepository {

    private static final String BASE = """
            SELECT line_id, record_id, subject_id, subject_code, subject_name, subject_type,
                   subject_content, subject_standard, mainten_status, mainten_result,
                   attr1, attr2, attr3, attr4,
                   create_by, create_time, update_by, update_time, remark
            FROM dv_mainten_record_line
            """;

    private final JdbcTemplate jdbc;

    public DvMaintenRecordLineRepository(JdbcTemplate jdbc) {
        this.jdbc = jdbc;
    }
    ...
    public List<Map<String, Object>> search(Map<String, String> params, int offset, int limit) {
        ...
        args.add(limit);
        args.add(offset);
        return jdbc.query(sql.toString(), (rs, n) -> DvSchemas.mapMaintenRecordLine(rs), args.toArray());
    }

    public long count(Map<String, String> params) {
        StringBuilder sql = new StringBuilder("SELECT COUNT(*) FROM dv_mainten_record_line WHERE 1=1 ");
        List<Object> args = new ArrayList<>();
        appendFilters(sql, args, params);
    ...
    public Optional<Map<String, Object>> findById(Long id) {
        List<Map<String, Object>> rows = jdbc.query(BASE + " WHERE line_id = ?",
                (rs, n) -> DvSchemas.mapMaintenRecordLine(rs), id);
        return rows.stream().findFirst();
    }

    public int deleteByRecordId(Long recordId) {
        return jdbc.update("DELETE FROM dv_mainten_record_line WHERE record_id = ?", recordId);
    }
        ...
        for (Map<String, Object> s : subjects) {
            jdbc.update("""
                    INSERT INTO dv_mainten_record_line (
                      ...
                      subject_content, subject_standard, mainten_status, create_time
                    ) VALUES (?,?,?,?,?,?,?,?,NOW(3))
                    """,
                    recordId,
    ...
    public Long insert(Map<String, Object> body) {
        ...
        jdbc.update(con -> {
            PreparedStatement ps = con.prepareStatement("""
                    INSERT INTO dv_mainten_record_line (
                      record_id, subject_id, subject_code, subject_name, subject_type,
                      subject_content, subject_standard, mainten_status, mainten_result, create_time
                    ) VALUES (?,?,?,?,?,?,?,?,?,?)
            ...
            ps.setString(i++, DvJdbcHelper.strOr(body.get("maintenStatus"), "Y"));
            ps.setString(i++, DvJdbcHelper.str(body.get("maintenResult")));
    ...
    public int update(Map<String, Object> body) {
        return jdbc.update("""
                UPDATE dv_mainten_record_line SET
                  record_id=?, subject_id=?, subject_code=?, subject_name=?, subject_type=?,
                  subject_content=?, subject_standard=?, mainten_status=?, mainten_result=?,
                ...
                DvJdbcHelper.str(body.get("maintenStatus")),
                DvJdbcHelper.str(body.get("maintenResult")),
    ...
    public int deleteByIds(List<Long> ids) {
        ...
        return jdbc.update("DELETE FROM dv_mainten_record_line WHERE line_id IN (" + ph + ")", ids.toArray());
    }

...
@Repository
public class DvMaintenRecordRepository {
    ...
    private static final String BASE = """
            SELECT record_id, plan_id, plan_code, plan_name, plan_type,
                   ...
                   mainten_time, user_id, user_name, nick_name, status,
                   remark, attr1, attr2, attr3, attr4,
                   create_by, create_time, update_by, update_time
            FROM dv_mainten_record
            """;

    private final JdbcTemplate jdbc;

    public DvMaintenRecordRepository(JdbcTemplate jdbc) {
        this.jdbc = jdbc;
    }
    ...
    public List<Map<String, Object>> search(Map<String, String> params, int offset, int limit) {
        ...
        args.add(limit);
        args.add(offset);
        return jdbc.query(sql.toString(), (rs, n) -> DvSchemas.mapMaintenRecord(rs), args.toArray());
    }

    public long count(Map<String, String> params) {
        StringBuilder sql = new StringBuilder("SELECT COUNT(*) FROM dv_mainten_record WHERE 1=1 ");
        List<Object> args = new ArrayList<>();
        appendFilters(sql, args, params);
    ...
    public Optional<Map<String, Object>> findById(Long id) {
        List<Map<String, Object>> rows = jdbc.query(BASE + " WHERE record_id = ?",
                (rs, n) -> DvSchemas.mapMaintenRecord(rs), id);
        ...
        jdbc.update(con -> {
            PreparedStatement ps = con.prepareStatement("""
                    INSERT INTO dv_mainten_record (
                      ...
                      machinery_id, machinery_code, machinery_name, machinery_brand, machinery_spec,
                      mainten_time, user_id, user_name, nick_name, status, remark, create_by, create_time
                    ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
            ...
            ps.setTimestamp(i++, DvJdbcHelper.parseTs(body.get("maintenTime")));
    ...
    public int update(Map<String, Object> body) {
        return jdbc.update("""
                UPDATE dv_mainten_record SET
                  plan_id=?, plan_code=?, plan_name=?, plan_type=?,
                  machinery_id=?, machinery_code=?, machinery_name=?, machinery_brand=?, machinery_spec=?,
                  mainten_time=?, user_id=?, user_name=?, nick_name=?, status=?, remark=?,
                ...
                DvJdbcHelper.parseTs(body.get("maintenTime")),
    ...
    public int deleteByIds(List<Long> ids) {
        ...
        return jdbc.update("DELETE FROM dv_mainten_record WHERE record_id IN (" + ph + ")", ids.toArray());
    }

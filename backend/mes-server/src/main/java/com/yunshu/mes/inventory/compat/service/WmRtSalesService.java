...
@Service
public class WmRtSalesService {

    private final JdbcTemplate jdbc;
    private final StorageCoreService storageCore;

    public WmRtSalesService(JdbcTemplate jdbc, StorageCoreService storageCore) {
        this.jdbc = jdbc;
    ...
    public List<Map<String, Object>> listHeader(Map<String, String> params) {
        ...
        return WmSqlHelper.list(jdbc, "wm_rt_sales", "rt_id", WmDocSchemas.RT_SALES,
                WmDocSchemas.filter(params, "rtCode", "rtName", "clientName", "soCode", "status"),
                PageUtil.offset(pn, ps), ps);
    }

    public long countHeader(Map<String, String> params) {
        return WmSqlHelper.count(jdbc, "wm_rt_sales", WmDocSchemas.RT_SALES,
                WmDocSchemas.filter(params, "rtCode", "rtName", "clientName", "soCode", "status"));
    }

    public Map<String, Object> getHeader(Long id) {
        return WmSqlHelper.getById(jdbc, "wm_rt_sales", "rt_id", "rtId", WmDocSchemas.RT_SALES, id);
    }

    @Transactional
    public Long createHeader(Map<String, Object> body) {
        return WmSqlHelper.insert(jdbc, "wm_rt_sales", WmDocSchemas.RT_SALES, body);
    }

    @Transactional
    public int updateHeader(Map<String, Object> body) {
        Long id = Long.parseLong(String.valueOf(body.get("rtId")));
        return WmSqlHelper.update(jdbc, "wm_rt_sales", "rt_id", WmDocSchemas.RT_SALES, body, id);
    }
    ...
    public int deleteHeader(Long id) {
        ...
        jdbc.update("DELETE FROM wm_rt_sales_detail WHERE rt_id = ?", id);
        jdbc.update("DELETE FROM wm_rt_sales_line WHERE rt_id = ?", id);
        return WmSqlHelper.delete(jdbc, "wm_rt_sales", "rt_id", id);
    }
    ...
    public void execute(Long rtId) {
        ...
        if (!"APPROVED".equals(h.get("status"))) {
            throw new IllegalArgumentException("只有待执行状态的单据才能执行退货入库");
        }
        Long lines = jdbc.queryForObject("SELECT COUNT(*) FROM wm_rt_sales_line WHERE rt_id = ?", Long.class, rtId);
        if (lines == null || lines == 0) {
            throw new IllegalArgumentException("请添加销售退货单行");
        }
        Long details = jdbc.queryForObject("SELECT COUNT(*) FROM wm_rt_sales_detail WHERE rt_id = ?", Long.class, rtId);
        if (details == null || details == 0) {
            throw new IllegalArgumentException("请添加退货明细");
        }
        storageCore.processRtSales(rtId);
        jdbc.update("UPDATE wm_rt_sales SET status = 'FINISHED', update_time = NOW() WHERE rt_id = ?", rtId);
    }

    public List<Map<String, Object>> listLine(Map<String, String> params) {
        int pn = PageUtil.pageNum(params);
        int ps = PageUtil.pageSize(params);
        return WmSqlHelper.list(jdbc, "wm_rt_sales_line", "line_id", WmDocSchemas.RT_SALES_LINE,
                WmDocSchemas.filter(params, "rtId"), PageUtil.offset(pn, ps), ps);
    }

    public long countLine(Map<String, String> params) {
        return WmSqlHelper.count(jdbc, "wm_rt_sales_line", WmDocSchemas.RT_SALES_LINE,
    ...
    public Map<String, Object> getLine(Long id) {
        return WmSqlHelper.getById(jdbc, "wm_rt_sales_line", "line_id", "lineId", WmDocSchemas.RT_SALES_LINE, id);
    }

    @Transactional
    public Long createLine(Map<String, Object> body) {
        return WmSqlHelper.insert(jdbc, "wm_rt_sales_line", WmDocSchemas.RT_SALES_LINE, body);
    }

    @Transactional
    public int updateLine(Map<String, Object> body) {
        Long id = Long.parseLong(String.valueOf(body.get("lineId")));
        return WmSqlHelper.update(jdbc, "wm_rt_sales_line", "line_id", WmDocSchemas.RT_SALES_LINE, body, id);
    }

    @Transactional
    public int deleteLine(Long id) {
        jdbc.update("DELETE FROM wm_rt_sales_detail WHERE line_id = ?", id);
        return WmSqlHelper.delete(jdbc, "wm_rt_sales_line", "line_id", id);
    }

    public List<Map<String, Object>> listDetail(Map<String, String> params) {
        int pn = PageUtil.pageNum(params);
        int ps = PageUtil.pageSize(params);
        return WmSqlHelper.list(jdbc, "wm_rt_sales_detail", "detail_id", WmDocSchemas.RT_SALES_DETAIL,
                WmDocSchemas.filter(params, "rtId", "lineId"), PageUtil.offset(pn, ps), ps);
    }

    public long countDetail(Map<String, String> params) {
        return WmSqlHelper.count(jdbc, "wm_rt_sales_detail", WmDocSchemas.RT_SALES_DETAIL,
                WmDocSchemas.filter(params, "rtId", "lineId"));
    }

    public Map<String, Object> getDetail(Long id) {
        return WmSqlHelper.getById(jdbc, "wm_rt_sales_detail", "detail_id", "detailId", WmDocSchemas.RT_SALES_DETAIL, id);
    }

    @Transactional
    public Long createDetail(Map<String, Object> body) {
        return WmSqlHelper.insert(jdbc, "wm_rt_sales_detail", WmDocSchemas.RT_SALES_DETAIL, body);
    }

    @Transactional
    public int updateDetail(Map<String, Object> body) {
        Long id = Long.parseLong(String.valueOf(body.get("detailId")));
        return WmSqlHelper.update(jdbc, "wm_rt_sales_detail", "detail_id", WmDocSchemas.RT_SALES_DETAIL, body, id);
    }

    @Transactional
    public int deleteDetail(Long id) {
        return WmSqlHelper.delete(jdbc, "wm_rt_sales_detail", "detail_id", id);
    }
}

...
@Repository
public class DvCheckMachineryRepository {

    private static final String BASE = """
            SELECT record_id, plan_id, machinery_id, machinery_code, machinery_name,
                   machinery_brand, machinery_spec,
                   remark, attr1, attr2, attr3, attr4,
                   create_by, create_time, update_by, update_time
            FROM dv_check_machinery
            """;

    private final JdbcTemplate jdbc;

    public DvCheckMachineryRepository(JdbcTemplate jdbc) {
        this.jdbc = jdbc;
    }
    ...
    public List<Map<String, Object>> search(Map<String, String> params, int offset, int limit) {
        ...
        args.add(limit);
        args.add(offset);
        return jdbc.query(sql.toString(), (rs, n) -> DvSchemas.mapCheckMachinery(rs), args.toArray());
    }

    public long count(Map<String, String> params) {
        StringBuilder sql = new StringBuilder("SELECT COUNT(*) FROM dv_check_machinery WHERE 1=1 ");
        List<Object> args = new ArrayList<>();
        appendFilters(sql, args, params);
    ...
    public Optional<Map<String, Object>> findById(Long id) {
        List<Map<String, Object>> rows = jdbc.query(BASE + " WHERE record_id = ?",
                (rs, n) -> DvSchemas.mapCheckMachinery(rs), id);
        return rows.stream().findFirst();
    }

    public List<Long> findPlanIdsByMachineryCode(String machineryCode) {
        if (!StringUtils.hasText(machineryCode)) {
            return List.of();
        }
        return jdbc.query("SELECT DISTINCT plan_id FROM dv_check_machinery WHERE machinery_code = ?",
                (rs, n) -> rs.getLong("plan_id"), machineryCode.trim());
    }

    public int deleteByPlanId(Long planId) {
        return jdbc.update("DELETE FROM dv_check_machinery WHERE plan_id = ?", planId);
    }
        ...
        jdbc.update(con -> {
            PreparedStatement ps = con.prepareStatement("""
                    INSERT INTO dv_check_machinery (
                      plan_id, machinery_id, machinery_code, machinery_name,
                      machinery_brand, machinery_spec, remark, create_by, create_time
                    ) VALUES (?,?,?,?,?,?,?,?,?)
    ...
    public int update(Map<String, Object> body) {
        return jdbc.update("""
                UPDATE dv_check_machinery SET
                  plan_id=?, machinery_id=?, machinery_code=?, machinery_name=?,
                  machinery_brand=?, machinery_spec=?, remark=?,
    ...
    public int deleteByIds(List<Long> ids) {
        ...
        return jdbc.update("DELETE FROM dv_check_machinery WHERE record_id IN (" + ph + ")", ids.toArray());
    }

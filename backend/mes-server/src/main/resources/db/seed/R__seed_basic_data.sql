...
VALUES
  ...
  ('TESTER', '测试人员', '超级管理员：全系统页面访问与全模块操作权限')
...
VALUES
  ...
  ('tester', '{noop}123456', 'U0099', '测试人员', (SELECT dept_id FROM sys_department WHERE dept_code = 'MGMT'), 'ENABLED')
...
INSERT IGNORE INTO sys_user_role (user_id, role_id)
SELECT u.user_id, r.role_id
FROM sys_user u JOIN sys_role r ON r.role_code = 'TESTER'
...
INSERT INTO production_line (workshop_id, line_code, line_name, rated_capacity, capacity_unit)
VALUES
  ((SELECT workshop_id FROM workshop WHERE workshop_code = 'WS-FAN-01'), 'LINE-FAN-02', '台扇组装二线', 600, 'PCS/DAY'),
  ((SELECT workshop_id FROM workshop WHERE workshop_code = 'WS-FAN-01'), 'LINE-FAN-03', '风扇组装线 A-01', 500, 'PCS/DAY')
ON DUPLICATE KEY UPDATE line_name = VALUES(line_name), rated_capacity = VALUES(rated_capacity);

INSERT INTO workstation (line_id, station_code, station_name, station_type)
VALUES
  ((SELECT line_id FROM production_line WHERE line_code = 'LINE-FAN-02'), 'ST-05', '底座装配工位', 'ASSEMBLY'),
  ((SELECT line_id FROM production_line WHERE line_code = 'LINE-FAN-02'), 'ST-06', '整机测试工位', 'TEST'),
  ((SELECT line_id FROM production_line WHERE line_code = 'LINE-FAN-03'), 'ST-07', '电机装配工位', 'ASSEMBLY'),
  ((SELECT line_id FROM production_line WHERE line_code = 'LINE-FAN-03'), 'ST-08', '包装工位', 'PACKAGE')
ON DUPLICATE KEY UPDATE station_name = VALUES(station_name), station_type = VALUES(station_type);

INSERT INTO customer_order (order_no, customer_name, order_date, delivery_date, status, created_by)
VALUES ('CO-20260701', '华东家电连锁', '2026-07-01', '2026-07-15', 'CONFIRMED',
...
INSERT INTO work_order (work_order_no, order_id, product_id, bom_id, route_id, plan_qty, completed_qty, status, planned_start_time, actual_start_time)
VALUES
  ('WO-20260701',
   (SELECT order_id FROM customer_order WHERE order_no = 'CO-20260701'),
   ...
   500, 320, 'RUNNING', NOW(), NOW()),
  ('WO-20260702', NULL,
   (SELECT product_id FROM product WHERE product_code = 'FAN-FS40-A'),
   (SELECT bom_id FROM bom WHERE bom_code = 'BOM-FAN-FS40-A' AND version_no = 'V1.0'),
   (SELECT route_id FROM process_route WHERE route_code = 'ROUTE-FS40-A' AND version_no = 'V1.0'),
   300, 180, 'RUNNING', NOW(), NOW()),
  ('WO-20260703', NULL,
   (SELECT product_id FROM product WHERE product_code = 'FAN-FS40-A'),
   (SELECT bom_id FROM bom WHERE bom_code = 'BOM-FAN-FS40-A' AND version_no = 'V1.0'),
   (SELECT route_id FROM process_route WHERE route_code = 'ROUTE-FS40-A' AND version_no = 'V1.0'),
   200, 95, 'DISPATCHED', NOW(), NULL)
...
VALUES
  ('PT-20260701',
   (SELECT work_order_id FROM work_order WHERE work_order_no = 'WO-20260701'),
   (SELECT line_id FROM production_line WHERE line_code = 'LINE-FAN-01'),
   (SELECT shift_id FROM factory_shift WHERE shift_code = 'DAY'),
   CURDATE(), 500, 320, 'RUNNING', NOW()),
  ('PT-20260702',
   (SELECT work_order_id FROM work_order WHERE work_order_no = 'WO-20260702'),
   (SELECT line_id FROM production_line WHERE line_code = 'LINE-FAN-02'),
   (SELECT shift_id FROM factory_shift WHERE shift_code = 'DAY'),
   CURDATE(), 300, 180, 'RUNNING', NOW()),
  ('PT-20260703',
   ...
   (SELECT shift_id FROM factory_shift WHERE shift_code = 'DAY'),
   CURDATE(), 200, 95, 'DISPATCHED', NOW())
ON DUPLICATE KEY UPDATE task_qty = VALUES(task_qty), completed_qty = VALUES(completed_qty), status = VALUES(status);

INSERT INTO dispatch_task (dispatch_no, task_id, work_order_id, step_id, station_id, operator_id, planned_qty, completed_qty, status, actual_start_time)
VALUES
  ('DT-20260701',
   (SELECT task_id FROM production_task WHERE task_no = 'PT-20260701'),
   (SELECT work_order_id FROM work_order WHERE work_order_no = 'WO-20260701'),
  ...
  ('DT-20260703',
   (SELECT task_id FROM production_task WHERE task_no = 'PT-20260701'),
   (SELECT work_order_id FROM work_order WHERE work_order_no = 'WO-20260701'),
   (SELECT step_id FROM process_step WHERE step_code = 'STEP-AGING'),
   (SELECT station_id FROM workstation WHERE station_code = 'ST-03'),
   (SELECT user_id FROM sys_user WHERE username = 'worker01'),
   125, 45, 'RUNNING', NOW()),
  ('DT-20260704',
   (SELECT task_id FROM production_task WHERE task_no = 'PT-20260702'),
   (SELECT work_order_id FROM work_order WHERE work_order_no = 'WO-20260702'),
   (SELECT step_id FROM process_step WHERE step_code = 'STEP-MOTOR'),
...
VALUES
  ('AD-20260701',
   (SELECT andon_type_id FROM andon_type WHERE type_code = 'MATERIAL_SHORTAGE'),
   (SELECT reason_id FROM andon_reason WHERE reason_code = 'MAT_LACK'),
   (SELECT work_order_id FROM work_order WHERE work_order_no = 'WO-20260701'),
   (SELECT line_id FROM production_line WHERE line_code = 'LINE-FAN-01'),
   (SELECT station_id FROM workstation WHERE station_code = 'ST-01'),
   (SELECT user_id FROM sys_user WHERE username = 'worker01'),
   'M4螺丝库存不足', 'OPEN', NOW()),
  ('AD-20260702',
   (SELECT andon_type_id FROM andon_type WHERE type_code = 'QUALITY_ABNORMAL'),
   (SELECT reason_id FROM andon_reason WHERE reason_code = 'NOISE_ABNORMAL'),
   (SELECT work_order_id FROM work_order WHERE work_order_no = 'WO-20260703'),
   (SELECT line_id FROM production_line WHERE line_code = 'LINE-FAN-03'),
   (SELECT station_id FROM workstation WHERE station_code = 'ST-07'),
   (SELECT user_id FROM sys_user WHERE username = 'worker01'),
   '设备噪音异常', 'OPEN', NOW())
ON DUPLICATE KEY UPDATE exception_desc = VALUES(exception_desc), status = VALUES(status);

-- device_oee_daily removed in V29; OEE demo data lives in dv compat seeds

INSERT INTO production_report (report_no, dispatch_id, work_order_id, step_id, station_id, operator_id, good_qty, report_time)
VALUES
  ('PR-20260701-H8',
   (SELECT dispatch_id FROM dispatch_task WHERE dispatch_no = 'DT-20260701'),
   (SELECT work_order_id FROM work_order WHERE work_order_no = 'WO-20260701'),
   (SELECT step_id FROM process_step WHERE step_code = 'STEP-MOTOR'),
   (SELECT station_id FROM workstation WHERE station_code = 'ST-01'),
   (SELECT user_id FROM sys_user WHERE username = 'worker01'),
   12, CONCAT(CURDATE(), ' 08:30:00')),
  ('PR-20260701-H9',
   (SELECT dispatch_id FROM dispatch_task WHERE dispatch_no = 'DT-20260701'),
   (SELECT work_order_id FROM work_order WHERE work_order_no = 'WO-20260701'),
   (SELECT step_id FROM process_step WHERE step_code = 'STEP-MOTOR'),
  ...
  ('PR-20260701-H10',
   ...
   (SELECT station_id FROM workstation WHERE station_code = 'ST-01'),
   (SELECT user_id FROM sys_user WHERE username = 'worker01'),
   45, CONCAT(CURDATE(), ' 10:50:00'))
ON DUPLICATE KEY UPDATE good_qty = VALUES(good_qty);

INSERT INTO task_operation_log (task_id, dispatch_id, operation_type, operator_id, operation_time, remark)
VALUES (
  (SELECT task_id FROM production_task WHERE task_no = 'PT-20260701'),
  (SELECT dispatch_id FROM dispatch_task WHERE dispatch_no = 'DT-20260701'),
  'START',
